...
def render_quote_tab(bundle: DataBundle) -> None:
    ...
    if st.button("➕ 添加到报价单"):
        ...
        process_summary = ",".join([p["name"] for p in process_breakdown]) if process_breakdown else "-"
        line = {
            "line_id": line_id,
            "sku": template.get("sku"),
            "product_name": template.get("name"),
            "qty": int(qty),
            "material_code": template.get("material_code"),
            "unit_price": summary["unit_price"],
            "line_total": summary["final_price_total"],
            ...
            "packaging_cost": summary["packaging_cost"],
            "shipping_alloc": 0.0,
        }
    ...
    st.dataframe(pd.DataFrame(quote_lines), use_container_width=True)
    ...
    if quote_lines:
        ...
        m1.metric("Lines Subtotal", format_currency(lines_subtotal, header_currency))
        m2.metric("Order Shipping", format_currency(order_shipping, header_currency))
        m3.metric("Final Total", format_currency(final_total, header_currency))

        st.markdown("**删除报价行**")
        option_map = {
            f"{line.get('line_id')} | {line.get('product_name')} | qty={line.get('qty')}": line.get("line_id")
            for line in quote_lines
        }
        selected_label = st.selectbox("选择要删除的行", list(option_map.keys()))
        if st.button("删除该行"):
            target_id = option_map[selected_label]
            st.session_state["quote_lines"] = [l for l in st.session_state["quote_lines"] if l.get("line_id") != target_id]
            st.success(f"已删除报价行 {target_id}")
